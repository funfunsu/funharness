...
7. **接线与孤儿检测**：
	- 凡产出新组件/新服务/新模块的任务，是否在 `改造输出`/`modifiedFiles` 列出了将其接入既有主干的调用方/引用方文件。
	- 设计集成接缝清单（Integration Points）中每条入边坐标，是否都能在某个任务的 `改造输出` 中找到对应接线交付物。
	- 任何“只新建文件、无任何改造输出”的新能力任务，视为孤儿交付，直接打回。
