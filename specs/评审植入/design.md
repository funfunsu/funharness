...
本设计面向“评审植入”能力，覆盖需求文档中的 Req-1 至 Req-4，目标是在需求、设计、测试用例三个关键阶段提供可选评审入口，并支持按阶段选择通用模板或用户自定义模板发起评审。

设计原则：
- 非阻断：评审为可选动作，不改变主流程保存/推进语义（Req-1, Req-4）。
- 分阶段：需求、设计、测试用例必须使用各自阶段模板，不允许跨阶段误用（Req-2, Req-3）。
...
```mermaid
flowchart TD
    A[Stage View: requirements/design/testcase] --> B[Review Entry Controller]
...
├── webviewTemplates.ts                # 三个关键阶段评审入口、状态展示与自定义模板编辑器（Req-1, Req-3, Req-4）
└── services/
...
路由约束：
- `openStageReview` 与 `runStageReview` 仅在 `requirements|design|testcase` 三个阶段生效（Req-1）。
- `saveStagePrompt` 与 `getLatestReviewStatus` 同样仅在 `requirements|design|testcase` 三个阶段生效（Req-1）。
- 非法阶段值由 `HarnessMessageController#ensureReviewStage` 拦截并提示 `REVIEW-VAL-001`，不抛出异常到主流程（Req-1, Req-4）。
- `runStageReview` 内必须先做模板解析，再创建评审请求（Req-2, Req-3）。
...
- API-1 `openStageReview -> stageReviewOpened`
  - request:
    - `stage: "requirements" | "design" | "testcase"`
...
模型约束：
- MODEL-1 的 `stage` 必须是三阶段枚举值之一，禁止跨阶段复用配置（Req-3）；持久化文件为 `.fun-harness/review-prompt-config.json`。
- MODEL-2 的 `defaultPrompt` 必须按阶段区分，不得三阶段共用同一未分阶段模板（Req-2）。
...
组件约束：
- `StageReviewEntryButton.visible=true` 仅在三关键阶段成立（Req-1）。
...
| --- | --- | --- | --- |
| INV-1 | Req-1 | project-structure-init | 在 `requirements|design|testcase` 阶段页面加载完成后，评审入口必须可见且默认不触发评审执行。 |
...
| 场景 | 处理策略 | 绑定需求 |
| --- | --- | --- |
| 阶段值非法（不在三关键阶段） | 在消息入口层拦截并提示 `REVIEW-VAL-001`，不下发评审调用；不影响主流程状态 | Req-1, Req-4 |
...
| --- | --- | --- | --- |
| TEST-1 | 三阶段入口可见且默认未执行 | GIVEN 进入任一关键阶段 WHEN 页面加载 THEN 可见评审入口且状态为 `idle` | [Req-1] |
...
| TEST-5 | 三阶段通用模板区分 | GIVEN 分别在三阶段发起评审 WHEN 构造请求 THEN 三阶段模板内容可区分 | [Req-2] |
...
apiContracts:
  - id: API-1
    ...
    request:
      stage: enum(requirements|design|testcase)
    emits:
      type: stageReviewOpened
      stage: enum(requirements|design|testcase)
      reviewEnabled: true
