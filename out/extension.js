...
class Harness {
    ...
    init() {
        ...
        try {
            ...
            this.projectStructureService = new projectStructureService_1.ProjectStructureService(workspaceRoot, extensionPath);
            this.aiDispatchService = new aiDispatchService_1.AiDispatchService(() => this.config);
            this.schedulerRegistry = new schedulerRegistry_1.SchedulerRegistry((task) => this.getIterationDir(task), workspaceRoot, () => this.config, async (query, iterDir, source, providerOverride) => this.aiDispatchService.dispatch(query, iterDir, source, providerOverride), () => this.render());
     ...
     */
    reconcileStagesWithArtifacts() {
        let dirty = false;
        for (const task of this.tasks) {
            if (task.stage === models_1.STAGE.DONE || task.stage === models_1.STAGE.READY_FOR_REVIEW) {
                continue;
            }
            ...
            const hasTaskPlan = fs.existsSync(path.join(iterDir, ...models_1.TASK_PLAN_PRIMARY_REL_PATH.split('/')))
                || fs.existsSync(path.join(iterDir, ...models_1.TASK_PLAN_LEGACY_REL_PATH.split('/')));
            let target = task.stage;
            if (hasTaskPlan) {
                target = models_1.STAGE.DEVELOPING;
            }
            else if (hasDesign) {
                target = models_1.STAGE.WRITING_TASKS;
            }
            else if (hasRequirements) {
                target = models_1.STAGE.WRITING_DESIGN;
            }
            // Only advance, never regress
            const stageOrder = [
                models_1.STAGE.INITIALIZING, models_1.STAGE.WRITING_REQUIREMENT, models_1.STAGE.WRITING_DESIGN,
                models_1.STAGE.WRITING_TESTCASE, models_1.STAGE.WRITING_TASKS, models_1.STAGE.DEVELOPING,
                models_1.STAGE.READY_FOR_REVIEW, models_1.STAGE.DONE,
            ];
    ...
    async handleSaveGit(frontendGit, backendGit, baseBranch, dryRun) {
    ...
    handleSaveDevConfig(msg) {
        ...
        const selectedProvider = (0, models_1.getAiProvider)(msg.ap);
        if (selectedProvider.kind === 'cli' && customCliTemplate && !customCliTemplate.includes('{promptFile}')) {
            vscode.window.showErrorMessage('CLI 命令模板必须包含 {promptFile} 占位符，例如：cat "{promptFile}" | claude。已拦截保存。');
            return;
        }
    ...
    ensureProjectStructureBaseline() {
    ...
    async handleInitProjectStructure() {
        if (this.configMeta.readOnly) {
        ...
        if (detected.detected) {
            const action = await vscode.window.showInformationMessage(`已生成项目结构预览（${detected.summary}）。请先检查并可直接编辑该预览，完成后点击“应用预览结构到正式文档”。`, '改用默认结构', '立即应用预览');
            if (action === '改用默认结构') {
    ...
    buildProjectStructureAiReviewPrompt(detectedContent, summary) {
    ...
    async handleApplyProjectStructurePreview() {
        if (this.configMeta.readOnly) {
    ...
    async handleOpenArtifactsIndex() {
    ...
    async handleOpenMasterWorkspace() {
    ...
    async detectOrGenerateFrontendStartCommand(frontendRoot) {
        ...
        const action = await vscode.window.showInformationMessage(`未检测到 scripts.dev。是否按项目依赖自动生成 scripts.dev = \"${generatedScript}\"？`, '生成并回填', '仅本次使用', '跳过');
        if (action === '生成并回填') {
    ...
    async detectOrGenerateBackendStartCommand(backendRoot) {
        const mode = this.resolveStartupChainMode(this.config.startupChainMode);
        ...
        const action = await vscode.window.showInformationMessage(`未检测到后端可用启动脚本。是否按项目依赖自动生成 scripts.dev = "${generatedScript}"？`, '生成并回填', '仅本次使用', '跳过');
        if (action === '生成并回填') {
    ...
    detectFrontendStartCommand(frontendRoot) {
        ...
        try {
            ...
            const inferred = this.inferRunScriptFromPackageScripts(scripts, ['watch', 'start:local', 'start-local', 'preview', 'compile'], [
                /\bvite\b/i,
                /next\s+dev/i,
                /nuxt\s+dev/i,
                /webpack\s+serve/i,
    ...
    suggestFrontendDevScript(frontendRoot) {
        const pkgPath = path.join(frontendRoot, 'package.json');
        if (!fs.existsSync(pkgPath)) {
            return '';
        }
    ...
    suggestBackendDevScript(backendRoot) {
        ...
        try {
            ...
            if (deps.nodemon)
                return 'nodemon src/index.js';
            if (deps['ts-node-dev'])
                return 'ts-node-dev --respawn src/index.ts';
    ...
    buildEnhancedNodeStartCommand(projectRoot, runCommand, mode, template) {
    ...
    getNodeInstallCommand(packageManager) {
    ...
    normalizeNodeRunCommand(command, packageManager) {
        if (!command) {
            return '';
        }
    ...
    writeDevScriptToPackageJson(frontendRoot, devScript) {
        const pkgPath = path.join(frontendRoot, 'package.json');
        if (!fs.existsSync(pkgPath)) {
            return false;
        }
    ...
    detectEnhancedJavaBackendStartCommand(backendRoot, mode, profile, template) {
        const profileArgs = (runner) => {
            if (!profile) {
                return '';
            }
            if (runner === 'maven') {
    ...
    resolveMavenBootRunSelector(backendRoot) {
        ...
        for (const modulePath of modulePaths) {
            ...
            try {
                ...
                if (hasBootStarter)
                    score += 30;
                if (/start|boot|app|web/.test(name))
                    score += 20;
    ...
    resolveStartupChainMode(mode) {
        return mode === 'light' ? 'light' : 'full';
    }
    applyStartupTemplate(template, vars, fallbackTemplate) {
    ...
    detectBackendNodeStartCommand(backendRoot) {
        const pkgPath = path.join(backendRoot, 'package.json');
        if (!fs.existsSync(pkgPath)) {
            return '';
        }
        try {
            ...
            if (scripts.start)
                return 'npm run start';
            const inferred = this.inferRunScriptFromPackageScripts(scripts, ['watch', 'serve', 'preview', 'compile'], [
    ...
    inferRunScriptFromPackageScripts(scripts, preferredKeys, commandMatchers) {
