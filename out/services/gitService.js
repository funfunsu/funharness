...
class GitService {
    ...
    async syncRepoToWorktree(mainRepoDir, worktreeDir, baseBranch, expectedBranch) {
    ...
    async mergeIterationToTarget(task, iterationDir, options = {}) {
     ...
     */
    async prepareIterationForMerge(worktreeDir, sourceBranch, task) {
        if (!fs.existsSync(worktreeDir)) {
            return { ok: false, reason: `worktree 目录不存在：${worktreeDir}` };
        }
    ...
    async mergeRepoBranch(repoDir, sourceBranch, targetBranch, expectedSourceSha) {
        if (!fs.existsSync(repoDir)) {
    ...
    async cleanupMergedBranch(mainRepoDir, worktreeDir, sourceBranch, targetBranch) {
        if (!fs.existsSync(mainRepoDir)) {
            return { ok: false, reason: `主仓库目录不存在：${mainRepoDir}` };
        }
        ...
        if (sourceShaOut.success) {
            ...
            if (sourceSha) {
                const ancestorOk = await this.execCmd(`git merge-base --is-ancestor ${sourceSha} origin/${targetBranch}`, mainRepoDir);
                if (!ancestorOk) {
                    return {
                        ok: false,
    ...
    async getCurrentBranch(repoDir) {
     ...
     */
    async assertExpectedBranch(repoDir, expectedBranch) {
        if (!fs.existsSync(repoDir)) {
            return null;
        }
